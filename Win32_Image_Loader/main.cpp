...
const unsigned int _kuiWINDOWWIDTH = 1280;
const unsigned int _kuiWINDOWHEIGHT = 720;
...
LRESULT CALLBACK WindowProc(HWND _hwnd, UINT _uiMsg, WPARAM _wparam, LPARAM _lparam)
{
	...
	case WM_COMMAND:
	{
		...
		case ID_FILE_LOADIMAGE:
		{
			if (ChooseImageFilesToLoad(_hwnd))
			{
				...
				MessageBox(_hwnd, g_vecImageFileNames.back().c_str(), L"Success Message", MB_OK);
				hBitMap = (HBITMAP)LoadImage(NULL, g_vecImageFileNames.back().c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
				//i got this from somewhere along the interwebs : doesn't display anything yet.... but also no errors
				/*HDC hdc = CreateCompatibleDC(NULL);
				HBITMAP bitmap = (HBITMAP)LoadImage(NULL, g_vecImageFileNames.front().c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
				...
				/*RECT getRekt;
				HDC hdc = GetDC(_hwnd);
				HBRUSH brush = CreatePatternBrush((HBITMAP)LoadImage(NULL, g_vecImageFileNames.front().c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE));
				GetWindowRect(_hwnd, &getRekt);
				FillRect(hdc, &getRekt, brush);
				DeleteObject(brush);
				ReleaseDC(_hwnd, hdc);*/
			}
			else
		...
		case ID_FILE_LOADSOUND:
		{
			if (ChooseSoundFilesToLoad(_hwnd))
			{
				...
				MessageBox(_hwnd, L"A Sound File is selected", L"Success Message", MB_OK);
			}
			else
			{
	...
	case WM_PAINT:
	{
		...
		BitBlt(_hWindowDC, 0, 0, bitmap.bmWidth, bitmap.bmHeight, hdcMem, 0, 0, SRCCOPY);
